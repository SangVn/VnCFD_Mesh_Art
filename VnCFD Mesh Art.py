...
# # VnCFD Mesh Art !
# Cùng với lưới có cấu trúc, lưới không cấu trúc được sử dụng rộng rãi trong các chương trình mô phỏng các hiện tượng vật lý như chuyển động phân tử, chất khí, chất lỏng, chất rắn; quá trình trao đổi nhiệt; các hiện tượng điện, từ trường; các quá trình phản ứng hóa học... Ngoài ra, ta có thể dùng nó để tạo ra những bức hình đẹp!
# 
# 
...
# Về cấu trúc, mỗi lưới là một tập hợp các ô lưới hình tam giác (triangle); mỗi tam giác được tạo thành bởi ba đỉnh (point, vertex, số nhiều: vertices), có ba cạnh (edge) và có tối đa 3 tam giác 'hàng xóm' (neighbor) theo thứ tự tương ứng với ba cạnh; mỗi cạnh gồm 2 đỉnh.
# 
...
# 
# Mask là một tập hợp các giá trị boolean độ dài bằng số lượng các tam giác. Nếu tam giác thứ $i$ được đánh dấu, nghĩa là $mask[i]=True$, thì nó sẽ không được biễu diễn khi plot. Ví dụ:
...
# tam giác thứ 1 ([1, 2, 5]) có hai hàng xóm là 4 và 3 theo thứ tự cạnh thứ 1 và 2, 
# cạnh 0 ko có hàng xóm là cạnh biên 
mesh.neighbors[0:2]
...
# # 2. Gmsh
# 
# Trường hợp có ít điểm lưới, ta có thể dựa theo cách trên để tự mình xác định tập hợp các tam giác (`triangles`). Tuy nhiên, nếu có cả trăm, nghìn điểm thì việc này quá tốn thời gian. Khi đó, có thể sử dụng `Triangulation` mà ko có biến `triangles`, nhưng có thể xuất hiện các tam giác không mong muốn nằm ngoài biên. Ví dụ: 
...
# Vậy nên, cách nhanh nhất là sử dụng các phần mềm chia lưới chuyên nghiệp. Ở đây chúng ta sẽ sử dụng **Gmsh (http://gmsh.info/)**. Đây là một phần mềm mã nguồn mở được sử dụng rộng rãi. Hãy cài đặt Gmsh, add path và xem qua cách sử trước khi thực hiện những bước tiếp theo!
# 
# ## Gmsh tutorials
# 
...
# 
# Ở đây, để chạy gmsh sẽ dùng các câu lệnh:
# <br>[http://gmsh.info/doc/texinfo/gmsh.html#Command_002dline-options]
# 
# **Ví dụ:**
# 
# câu lệnh để `mở file .geo, tạo lưới 2D và xuất lưới ở định dạng .dat`:
...
# Trong đó, `node` - chính là các điểm lưới, còn `element` là các tam giác. Ta viết một hàm để đọc `file lưới .msh` như sau:
...
def plot_gmsh(filename, plot_func=plot, colors=['r*-', 'm-', 'g-', 'y-'], cmaps=['spring', 'summer', 'winter']):
    points, triangles = read_gmsh(filename)
    mesh = mtri.Triangulation(points[:, 0], points[:, 1], triangles)
    plot_func(mesh, colors, cmaps)
...
# ## t2.geo
# 
# Chia lưới hình vuông khuyết ở giữa. Trong file `data/t2.geo` khai báo thêm đường viền bên trong, thêm một Curve loop.
...
plot_gmsh('data/t2.msh', plot_func=plot_border, cmaps=['autumn', 'plasma', 'ocean'])
...
# ...
# </pre>
# Trong đó `Element sets out` chính là lưới dải bao ngoài, `sets in` -- lưới hình vuông bên trong. 
# Để đọc sets, sửa lại hàm đọc lưới như sau:
...
# Hàm biểu diễn mới 
# Sử dụng mask để thay đổi màu sắc các vùng ô lưới (sets)
...
# 
# Đối với một miền không gian bất kì, đường biên là một tập hợp rất nhiều điểm, chúng có thể được lưu ở định dạng file `.dat` thông thường chỉ gồm `tọa độ (x, y)`. Ta sẽ viết thêm một hàm để chuyển từ file hình học `.dat` sang file input cho gmsh `.geo`. 
# 
...
# 
# Trường hợp khi có nhiều đường biên, trong file hình học .dat, ngoài tọa độ các điểm, ta cần chỉ rõ **có bao nhiêu đường biên, mỗi đường gồm bao nhiêu điểm, và viết luôn phần Plane Surface, Physical Surface vào đầu file**. Quy ước cách viết như ở ví dụ chia lưới cho profile (data/naca_23012.dat).
# 
...
def new_convert(infile, outfile, lc=0.0):
    ...
    with open(infile, 'r') as f:
        ...
        # đọc số điểm trên mỗi loop, xác định chỉ số của đường đầu tiên trong loop 
...
# Number of points of each curveloop: 19 37 48 40 22 17 4
...
# # Kết luận
# 
# Trên đây là các bước cơ bản để chia và biểu diễn lưới một miền không gian hai chiều bất kì. Bằng cách thay đổi các thông số color, cmap, mask bạn sẽ có được những hình ảnh đầy màu sắc.
# 
